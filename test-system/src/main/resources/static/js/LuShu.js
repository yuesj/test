...
(function() {
	 ...
	 * LuShu类的构造函数
	 * 
	 * @class LuShu <b>入口</b>。 实例化该类后，可调用,start,end,pause等方法控制覆盖物的运动。
	 * 
	 * @constructor
	 * @param {Map}
	 *            map Baidu map的实例对象.
	 ...
	 *            path 构成路线的point的数组.
	 * @param {Json
	 *            Object} opts 可选的输入参数，非必填项。可输入选项包括：<br /> {<br />"<b>landmarkPois</b>" :
	 *            {Array} 要在覆盖物移动过程中，显示的特殊点。格式如下:landmarkPois:[<br />
	 *            {lng:116.314782,lat:39.913508,html:'加油站',pauseTime:2},<br />
	 *            {lng:116.315391,lat:39.964429,html:'高速公路收费站,pauseTime:3}]<br />
	 *            <br />"<b>icon</b>" : {Icon} 覆盖物的icon, <br />"<b>speed</b>" :
	 *            {Number} 覆盖物移动速度，单位米/秒 <br />
	 *            <br />"<b>defaultContent</b>" : {String} 覆盖物中的内容 <br /> }<br />.
	 * @example <b>参考示例：</b><br />
	 *          var lushu = new
	 *          BMapLib.LuShu(map,arrPois,{defaultContent:"从北京到天津",landmarkPois:[]});
	 */
	BMapLib.LuShu = function(map, path, handle,opts) {
	 ...
	 *          lushu.start();
	 */
	LuShu.prototype.start = function(callback) {
	...
	baidu.object.extend(CustomOverlay.prototype, {
		...
		setPosition : function(poi, markerSize) {
			...
			var px = this._map.pointToOverlayPixel(poi), styleW = baidu.dom.getStyle(this._div, 'width'), styleH = baidu.dom.getStyle(this._div, 'height');
			overlayW = parseInt(this._div.clientWidth || styleW, 10), overlayH = parseInt(this._div.clientHeight || styleH, 10);
